...
function draw() {
  ...
  push();
  translate(110, 0);
  /*  (width - cell_x * tile_count) / 2,
    (height - cell_y * tile_count) / 2
  );*/
  background(hu, 40, 70);
  for (let i = 0; i < mySize * 1.1; i += cell_x) {
    noFill();
    stroke(90, 255, 255, 50);
    stroke(hu, 50, 100);
    strokeWeight(0.75);
    line(i, 0, i, height);
    line(0, i, width - 220, i);
  }
  Composition();
  picture_frame();
  pop();
...
function picture_frame() {
  ...
  if (width >= height) {
    noStroke();
    fill(10);
    rect(-height / 2, 0, width - mySize, height);
    //rect(width / 2, 0, width - mySize, height);
  } else {
    fill(10);
    rect(0, -height / 2, width, height - mySize);
    rect(0, height / 2, width, height - mySize);
  }
  pop();
}
